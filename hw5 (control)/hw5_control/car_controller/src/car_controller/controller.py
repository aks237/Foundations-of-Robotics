...
def compute_position_in_frame(p, frame):
    ...
    e_x = (np.cos(frame[2])*(p[0]-frame[0])) + (np.sin(frame[2])*(p[1]-frame[1]))
    e_y = (np.sin(frame[2])*(frame[0]-p[0])) + (np.cos(frame[2])*(p[1]-frame[1]))
    ep = np.empty(2)
    ep[0] = e_x
    ep[1] = e_y
    return ep
...
class BaseController(object):
    ...
    def get_reference_index(self, pose, path_xytv, distance_lookahead):
        ...
        with self.path_condition:
            ...
            #print("PRINTING: ", self.path_condition)
            distances = np.empty(path_xytv.shape[0])
            distances = (((path_xytv[:,0]-pose[0])**2)+((path_xytv[:,1]-pose[1])**2))**0.5
            min_index = np.argmin(distances)
            ref_index = min_index

            while(distances[ref_index] < distance_lookahead and ref_index < distances.size-1):
                ref_index += 1
