...
class ModelPredictiveController(BaseController):
    ...
    def sample_controls(self):
        ...
        angles = np.linspace(self.min_alpha, self.max_alpha, num=self.K)
        angles = np.reshape(angles, (self.K,1))
        controls[:, :, 1] = angles
    ...
    def get_rollout(self, pose, controls, dt=0.1):
        ...
        for i in range(self.T-1):
            rollouts[:, i+1, :] = rollouts[:, i, :] + self.motion_model.compute_changes(rollouts[:, i, :],controls[:, i, :],dt)
        # END SOLUTION
        return rollouts
    ...
    def compute_distance_cost(self, rollouts, reference_xyt):
        ...
        costs = np.empty(self.K)
        costs = ((((rollouts[:,self.T-1,0]-reference_xyt[0])**2) + ((rollouts[:,self.T-1,1]-reference_xyt[1])**2)) **0.5) * self.error_w
        return costs
        # END SOLUTION
    ...
    def compute_collision_cost(self, rollouts, _):
        ...
        # BEGIN SOLUTION "QUESTION 4.3" ALT="raise NotImplementedError"
        costs = np.empty(self.K)
        costs = np.sum(rollout_collisions,axis=1) * self.collision_w
        return costs
        # END SOLUTION
    ...
    def get_control(self, pose, reference_xytv, _):
        ...
        k_index = np.argmin(costs)
        initial_control = np.empty(2)
        initial_control = self.sampled_controls[k_index,0,:]
