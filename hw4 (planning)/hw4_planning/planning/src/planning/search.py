        ...
        while len(queue) > 0:
            ...
            for neighbor, w in self.rm.graph[entry.node].items():
                ...
                if not expanded[neighbor]:
                    queue.push(QueueEntry(h + weight + entry.cost_to_come, next(c),neighbor,entry.node,weight + entry.cost_to_come))
                ### END QUESTION 1.2 #####################
...
class RRTPlanner(object):
    ...
    def Plan(self, start_config, goal_config, epsilon=0.001):
        ...
        for _ in range(self.max_iter):
            ...
            for i in range(xs_rand.shape[0]):
                ...
                '''
                near_id, x_near = self.tree.GetNearestVertex(x_rand)
                x_new = self.extend(x_near,x_rand)
                if(x_new is not None):
                    new_id = self.tree.AddVertex(x_new)
                    new_eid.append(new_id)
                    self.tree.AddEdge(near_id, new_id)
                    if(epsilon > self.prob.compute_distance(x_new, self.end)):
                        found = True
                        goal_id = new_eid[-1]
                        break
    ...
    def extend(self, x_near, x_rand):
        ...
        '''
        ### BEGIN QUESTION 2.1 #####################
        step_size = self.eta * (x_rand - x_near)
        x_new = x_near + step_size
